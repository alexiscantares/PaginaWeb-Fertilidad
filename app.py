...
st.markdown("Plataforma de inferencia multivariada, interpretación metalogénica y reporte técnico.")
st.markdown("---")
...
# =====================================================================
# GESTIÓN DE API KEY DE GROQ (PERSISTENCIA EN SESSION_STATE / SECRETS)
# =====================================================================
...
if not st.session_state.groq_api_key:
    input_key = st.sidebar.text_input("Ingresa tu Groq API Key:", type="password", help="Se guardará durante tu sesión activa.")
    if input_key:
        st.session_state.groq_api_key = input_key
        st.sidebar.success("🔑 Key guardada correctamente.")
else:
    st.sidebar.success("🔑 API Key activa")
...
def popup_descargar_word(texto_reporte):
    st.write("Generando documento estructurado bajo formato formal...")
        ...
        with st.spinner('Procesando datos y ejecutando análisis multivariado...'):
            try:
                ...
                with tab4:
                    ...
                    if not st.session_state.groq_api_key:
                        st.warning("⚠️ Ingresa tu Groq API Key en el panel lateral para activar la generación automática del informe.")
                    else:
                        if st.button("📄 Sintetizar e Interpretar Informe NI 43-101"):
                            with st.spinner("Procesando síntesis con Llama 3.3 70B en Groq..."):
                                try:
                                    ...
                                    porcentaje_anomalias_tab = (muestras_fertiles_tab / total_muestras_tab) * 100
...
"""
                    ...
                    if 'reporte_groq' in st.session_state:
                        ...
                        # Botón que activa el Pop-up / Dialog modal para Word
